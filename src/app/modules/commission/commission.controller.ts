...
const getAgentTotalCommission = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const commissionSummary = await CommissionService.getAgentTotalCommission(
      agentId
    );
...
const getAgentCommissionTransactions = catchAsync(
  async (req: Request, res: Response) => {
    const verifiedToken = req.user as JwtPayload;
    const agentId = verifiedToken.userId;
    ...
    const result = await CommissionService.getAgentCommissionTransactions(
      agentId,
      page,
      limit,
      filters
...
const getAdminTotalCommission = catchAsync(
  async (req: Request, res: Response) => {
    const verifiedToken = req.user as JwtPayload;
...
const getAdminCommissionTransactions = catchAsync(
  async (req: Request, res: Response) => {
    const verifiedToken = req.user as JwtPayload;
    const userRole = verifiedToken.role;
    const page = parseInt(req.query.page as string) || 1;
    ...
    const result = await CommissionService.getAdminCommissionTransactions(
      page,
      limit,
      filters
    );
